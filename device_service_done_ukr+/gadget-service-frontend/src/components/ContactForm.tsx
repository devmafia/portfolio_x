...
const ContactForm: React.FC = () => {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-md">
      ...
      <button type="submit" className={`bg-primary text-white py-2 px-4 rounded-md ${isSubmitting ? 'opacity-50 cursor-not-allowed' : 'hover:bg-blue-700 transition'}`} disabled={isSubmitting}>Надіслати</button>
    </form>
  );
};
